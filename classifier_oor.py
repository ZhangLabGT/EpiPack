...
class global_oor_query():
    ...
    def _cal_dist_query(self, mu_set, cov_set):
        ...
        for i in range(len(mu_set)):
            ...
            dist_mat_query[:,i] = np.sqrt(np.diag(query_dist_i))

        return dist_mat_query
        
    def annotate(self,k = 15, confidence_threshold = 0.1, adjust_by_dist = True, adjust_threshold = 0.1):
        ...
        init_mu, init_cov_inv, dist_mat_ref = self._gaussian_init()
